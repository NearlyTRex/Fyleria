    ...
    const String& sWeaponSet) const
{
    ...
    {
        vEquippedItemTypes.push_back(GetManagers()->GetItemManager()->RetrieveItemType(item.GetItemTreeIndex()));
    }
    ...
    const CharacterAction& action) const
{
    ...
    {
        return DoesMeetItemUsedRequirements(GetManagers()->GetItemManager()->GetActionTypes(GetItemTreeIndex()));
    }
    ...
    StringArray& vDestCharIDs) const
{
    ...
    if(GetManagers()->GetCharacterPartyManager()->DoesPartyExistByType(sSourcePartyType))
    {
        GetManagers()->GetCharacterPartyManager()->GetPartyByType(sSourcePartyType).GetCharacterIDsFromTargetType(sSourceTargetType, vSourceCharIDs);
    }
    if(GetManagers()->GetCharacterPartyManager()->DoesPartyExistByType(sDestPartyType))
    {
        GetManagers()->GetCharacterPartyManager()->GetPartyByType(sDestPartyType).GetCharacterIDsFromTargetType(sDestTargetType, vDestCharIDs);
    }
...
const StatChangeArray& GetStatChangesFromSkillTreeIndex(const TreeIndex& treeIndex)
{
    ...
    switch(eSkillTreeType)
    {
        case SkillTreeType::Affinity:
            return GetManagers()->GetSkillManager()->RetrieveSkillDataAffinity(treeIndex).GetStatChanges();
        case SkillTreeType::Alchemy:
            return GetManagers()->GetSkillManager()->RetrieveSkillDataAlchemy(treeIndex).GetStatChanges();
        case SkillTreeType::Crafting:
            return GetManagers()->GetSkillManager()->RetrieveSkillDataCrafting(treeIndex).GetStatChanges();
        case SkillTreeType::Breakdown:
            return GetManagers()->GetSkillManager()->RetrieveSkillDataBreakdown(treeIndex).GetStatChanges();
        case SkillTreeType::Combat:
            return GetManagers()->GetSkillManager()->RetrieveSkillDataCombat(treeIndex).GetStatChanges();
        case SkillTreeType::Weapon:
            return GetManagers()->GetSkillManager()->RetrieveSkillDataWeapon(treeIndex).GetStatChanges();
        default:
            break;
    }
...
const StatChangeArray& GetStatChangesFromItemTreeIndex(const TreeIndex& treeIndex)
{
    ...
    switch(eItemTreeType)
    {
        case ItemTreeType::Armor:
            return GetManagers()->GetItemManager()->RetrieveItemDataArmor(treeIndex).GetStatChanges();
        case ItemTreeType::Ingredient:
            return GetManagers()->GetItemManager()->RetrieveItemDataIngredient(treeIndex).GetStatChanges();
        case ItemTreeType::Potion:
            return GetManagers()->GetItemManager()->RetrieveItemDataPotion(treeIndex).GetStatChanges();
        case ItemTreeType::Weapon:
            return GetManagers()->GetItemManager()->RetrieveItemDataWeapon(treeIndex).GetStatChanges();
        default:
            break;
    }
    THROW_RUNTIME_ERROR("Invalid or unknown tree index requested: " + treeIndex.GetTreeBranchLeafType());
