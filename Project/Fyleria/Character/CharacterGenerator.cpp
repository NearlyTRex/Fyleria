...
Bool CharacterGenerator::LoadFromFile(const String& sFilename, const String& sType)
{
    // Load from file
    Json jsonData;
    Bool bSuccess = ReadSerializedFile(sFilename, sType, jsonData, GetManagers()->GetFileManager()->GetDataGeneratorsDirectory());
    if(!bSuccess)
    {
        return false;
    }
...
Bool CharacterGenerator::SaveToFile(const String& sFilename, const String& sType)
{
    // Save to file
    return WriteSerializedFile(sFilename, sType, *this, GetManagers()->GetFileManager()->GetDataGeneratorsDirectory());
}
