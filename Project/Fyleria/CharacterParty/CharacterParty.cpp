...
Bool CharacterParty::AddMember(const String& sCharacterID)
{
    ...
    if(IsPartyFull())
    {
        ERROR_FORMAT_STATEMENT("Party at ID '{}' is full", GetPartyID().c_str());
        return false;
    }
    ...
    {
        ERROR_FORMAT_STATEMENT("Character at ID '{}' was already present in party", sCharacterID.c_str());
        return false;
    }
...
Bool CharacterParty::RemoveMember(const String& sCharacterID)
{
    ...
    if(!IsMemberPresent(sCharacterID))
    {
        ERROR_FORMAT_STATEMENT("Character at ID '{}' was not present in party", sCharacterID.c_str());
        return false;
    }
...
Bool CharacterParty::MoveMember(const String& sCharacterID, const String& sCharacterTargetType)
{
    ...
    {
        ERROR_FORMAT_STATEMENT("Target type '{}' is not available", sCharacterTargetType.c_str());
        return false;
    }
...
{
    for(auto& sTarget : GetAvailableTargetTypes())
    {
        return sTarget;
    }
    ...
    Int iAmountEnd)
{
    ...
    TreeIndexArray vAllArmors = GetManagers()->GetItemManager()->GetAllArmorItems();
    TreeIndexArray vAllIngredients = GetManagers()->GetItemManager()->GetAllIngredientItems();
    TreeIndexArray vAllPotions = GetManagers()->GetItemManager()->GetAllPotionItems();
    TreeIndexArray vAllWeapons = GetManagers()->GetItemManager()->GetAllWeaponItems();
    ...
    UInt uAmount)
{
    ...
    TreeIndex treeIndex = GetManagers()->GetItemManager()->ResolveItemLeafIntoIndex(sLeaf);
...
Bool CharacterParty::AddItemByTreeIndex(const TreeIndex& treeIndex, UInt uAmount)
{
    ...
    // Check if it exists
    if(!GetManagers()->GetItemManager()->DoesItemDataExist(treeIndex))
    {
    ...
    {
        String sItemType = GetManagers()->GetItemManager()->RetrieveItemType(treeIndex);
...
TreeIndex CharacterParty::GetBestUnequippedItem(const String& sCharacterID, const String& sSlot) const
{
    ...
    {
        if(uShieldCount == 1 && GetManagers()->GetItemManager()->IsItemShield(item.second.GetItemTreeIndex()))
        {
            continue;
        }
        ...
        Bool bIsArmor = GetManagers()->GetItemManager()->DoesItemDataArmorExist(item.second.GetItemTreeIndex());
        Bool bIsWeapon = GetManagers()->GetItemManager()->DoesItemDataWeaponExist(item.second.GetItemTreeIndex());
        if(
            (bIsArmor && GetManagers()->GetItemManager()->IsArmorBetter(item.second.GetItemTreeIndex(), bestItem)) ||
            (bIsWeapon && GetManagers()->GetItemManager()->IsWeaponBetter(item.second.GetItemTreeIndex(), bestItem)))
        {
            bestItem = item.second.GetItemTreeIndex();
        }
    }
    ...
    if(!bestItem.empty())
    {
        LOG_FORMAT_STATEMENT("Found item '{}'", bestItem.GetTreeBranchLeafType().c_str());
    }
#endif
    return bestItem;
}
...
Bool CharacterParty::EquipBestItems(const String& sCharacterID)
{
    ...
    for(auto& sEquipType : CharacterEquipmentType::_names())
    {
...
{
    LOG_FORMAT_STATEMENT("Resetting play time for party '{}'", GetPartyID().c_str());
...
{
    LOG_FORMAT_STATEMENT("Pausing play time for party '{}'", GetPartyID().c_str());
...
{
    LOG_FORMAT_STATEMENT("Resuming play time for party '{}'", GetPartyID().c_str());
    if (!GetPlayTimePaused())
    {
        return;
    }
